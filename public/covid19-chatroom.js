document.addEventListener('DOMContentLoaded', () => {
    ...
    const fetchMessages = async () => {
        try {
            const response = await fetch('/covid19-chatroom/get-messages');
    ...
    const sendMessage = async () => {
        const message = inputBox3.value;
        try {
            // Send message to server
            const response = await fetch('/covid19-chatroom/send-message', {
                method: 'POST',
                headers: {
    ...
    sendButton3.addEventListener('click', sendMessage);
